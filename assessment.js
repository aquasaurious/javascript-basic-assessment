...
function removeItem(strArr, str) {
	for (var i = strArr.length - 1; i >= 0; i--) { 
		if (strArr[i] == str) {                        // checking in reverse since .length is shrinking when we splice,
			strArr.splice(i,1);						  // in the other direction, items after a spliced item would miss the check
		}
	}
	return strArr;
}
...
function outside(temperature, humidity, cloudiness) {
	if (temperature > 80) {
		if (humidity > 40) return "I'm all sweat";
		if (humidity < 40 && cloudiness < 20) return "I'm literally in the desert";
	}
	if (temperature < 40 && cloudiness > 60) return "I have icecicles";
	else if (temperature > 80 || humidity > 50 || cloudiness > 50) return "Hmm, probably not";
